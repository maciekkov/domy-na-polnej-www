...
test('Publikacja aktualizuje stronę główną i SEO bez osobnych stron domów',()=>{
 ...
 try{mkdirSync(join(tmp,'assets/data'),{recursive:true});cpSync(join(root,'public/assets/data/asset-versions.json'),join(tmp,'assets/data/asset-versions.json'));cpSync(join(root,'index.html'),join(tmp,'index.html'));const d=copy();d.houses[0].status='Sprzedany';d.houses[1].price=765432;refreshPublishedSeo(tmp,d);const h=readFileSync(join(tmp,'index.html'),'utf8');assert.ok(h.includes('Już wkrótce'));assert.ok(!h.includes('765 432 zł'));assert.ok(!h.includes('779 000 zł'));assert.ok(h.includes('Sprzedany'));

...
test('Publikacja aktualizuje stronę główną i SEO bez osobnych stron domów',()=>{
 ...
 try{mkdirSync(join(tmp,'assets/data'),{recursive:true});cpSync(join(root,'public/assets/data/asset-versions.json'),join(tmp,'assets/data/asset-versions.json'));cpSync(join(root,'index.html'),join(tmp,'index.html'));const d=copy();d.houses[0].status='Sprzedany';d.houses[1].price=765432;refreshPublishedSeo(tmp,d);const h=readFileSync(join(tmp,'index.html'),'utf8');assert.ok(h.includes('765 432 zł'));assert.ok(!h.split('<section id="domy"')[0].includes('779 000 zł'));assert.ok(h.includes('Sprzedany')); // Historical sold price remains transparent in the no-JS list.
assert.ok(!h.includes('/dom-a/'));const once=readFileSync(join(tmp,'index.html'),'utf8');refreshPublishedSeo(tmp,d);assert.equal(readFileSync(join(tmp,'index.html'),'utf8'),once)}finally{rmSync(tmp,{recursive:true,force:true})}
})
...
test('Jeden katalog eventów JS/PHP, realny hook PDF i odsłony karty',()=>{const names=JSON.parse(read('api/event-names.json'));assert.ok(names.length>=18);assert.ok(read('src/lib/analytics.ts').includes('../../api/event-names.json'));assert.ok(read('api/analytics.php').includes('/event-names.json'));const card=read('src/components/house-selector/HouseCard.tsx');assert.ok(card.includes("track('house_pdf_download'"));assert.ok(card.includes("trackOnce('house_card_open'"));assert.ok(read('src/components/house-selector/HouseModal.tsx').includes('useDialog(ref,onClose)'));assert.ok(read('src/sections/Homes/Homes.tsx').includes('max-width: 960px'))})

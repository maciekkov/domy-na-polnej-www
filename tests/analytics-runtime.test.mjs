...
function fixture({local=false,storageThrows=false,setThrows=false,beacon=true,legacy=null}={}) {
  ...
  const document={referrer:'',visibilityState:'visible',addEventListener(name,fn){(docListeners[name]??=[]).push(fn)},get cookie(){return [...cookies].map(([k,v])=>`${k}=${encodeURIComponent(v)}`).join('; ')},set cookie(value){const first=value.split(';')[0],at=first.indexOf('='),name=decodeURIComponent(first.slice(0,at)),val=decodeURIComponent(first.slice(at+1));const max=/Max-Age=(\d+)/i.exec(value);if(max&&Number(max[1])===0)cookies.delete(name);else cookies.set(name,val)}}

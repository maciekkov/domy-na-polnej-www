...
for(const width of [320,390,430,760,1440]){
 ...
 if(width<=760){
 await expect(page.locator('.hero__bottom')).toBeHidden();await expect(page.locator('.hero__tour-link')).toBeHidden()
 ...
 }else{await expect(page.locator('.hero__bottom')).toBeVisible();await expect(page.locator('.homes__showcase>.house-card')).toBeVisible()}

...
const files=walk(dir), compiled=files.filter(f=>/\.(js|css)$/.test(f)).map(f=>readFileSync(f,'utf8')).join('\n')
...
console.log('PASS dist: brak panelu/CRM demo i sekretów; komplet danych, obrazów, API, stron prawnych i nagłówków cache.')

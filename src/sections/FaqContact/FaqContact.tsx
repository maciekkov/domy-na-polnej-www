...
export function FaqContact({ selectedHouse, contact }: Props) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (fields.name.trim().length < 2 || phone.length < 7 || !fields.consentContact || !fields.consentPrivacy) {
      setFormState('error')
    ...
    try {
      ...
      if (localPreview) {
        recordDemoLead({ name: fields.name.trim(), phone: fields.phone.trim(), email: fields.email.trim(), houseCode: fields.house, message: fields.message.trim() })
      } else {
      ...
      <section id="faq" className="faq-section" aria-labelledby="faq-title">
        <div className="shell faq-section__grid">
          <div className="faq-section__intro">
            ...
            <p>Tu znajdziesz odpowiedzi na najczęściej zadawane pytania. Jeśli nie znajdziesz tego, czego szukasz — skontaktuj się z nami. Chętnie porozmawiamy i rozwiejemy wszelkie wątpliwości.</p>
            <figure className="faq-section__image">
              <img src={`${import.meta.env.BASE_URL}assets/images/gallery/front-angle.webp`} alt="Dom na Polnej — widok elewacji i strefy wejściowej" width="900" height="600" loading="lazy" decoding="async" />
              <figcaption><Leaf aria-hidden="true" /><strong>Pewne odpowiedzi.<br />Spokojna decyzja.</strong></figcaption>
            </figure>
          </div>
            ...
            {faqItems.map((item) => {
              ...
              return (
                <article className={`faq-item ${open ? 'is-open' : ''}`} key={item.id} role="listitem">
                  ...
                  <div id={`faq-answer-${item.id}`} className="faq-item__answer" hidden={!open}>
                    <p>{item.answer}</p>
                  </div>
                </article>
              )
        ...
        <div className="shell contact-section__grid">
          <div className="contact-section__intro">
            ...
            <address className="contact-details">
              ...
              <a href={contact.emailHref}><Mail aria-hidden="true" /><span><strong>{contact.email}</strong><small>Odpowiadamy zwykle w ciągu 24h</small></span></a>
              <a href={contact.mapHref} target="_blank" rel="noreferrer"><MapPin aria-hidden="true" /><span><strong>{contact.addressLine1}</strong><small>{contact.addressLine2}</small><u>Zobacz na mapie →</u></span></a>
            </address>
            <div className="contact-section__botanical" aria-hidden="true">
              <img src={`${import.meta.env.BASE_URL}assets/images/botanical-corner.svg`} alt="" />
            </div>
          </div>
          ...
          <form className="contact-form" onSubmit={submit} noValidate onFocus={() => { if (!started) { setStarted(true); track('contact_start', fields.house) } }}>
            ...
            <label className="contact-form__wide">E-mail <small>(opcjonalnie)</small>
              <input value={fields.email} onChange={(event) => update('email', event.target.value)} type="email" autoComplete="email" placeholder="Twój e-mail" />
            </label>
            ...
            <label className="contact-form__consent contact-form__wide"><input type="checkbox" checked={fields.consentPrivacy} onChange={(event) => update('consentPrivacy', event.target.checked)} required /><span>Akceptuję politykę prywatności.</span></label>

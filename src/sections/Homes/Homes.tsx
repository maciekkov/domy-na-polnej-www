    ...
    <section className="homes" id="domy" aria-labelledby="homes-title">
      <div className="shell">
        ...
        <div className="homes__showcase">
          <Masterplan houses={houses} selectedId={selectedId} hoveredId={hoveredId} onHover={setHoveredId} onSelect={onSelect} />
        ...
        <div className="homes-table-wrap">
          <table className="homes-table">
            ...
            <thead><tr><th>Dom</th><th>Nr działki</th><th>Status</th><th>Powierzchnia</th><th>Działka</th><th>Pokoje</th><th>Cena</th></tr></thead>
            <tbody>
              {houses.map((house) => (
                <tr key={house.id} className={activeId === house.id ? 'is-active' : ''} onMouseEnter={() => setHoveredId(house.id)} onMouseLeave={() => setHoveredId(null)} onFocus={() => setHoveredId(house.id)} onBlur={() => setHoveredId(null)} onClick={() => onSelect(house.id)}>
                  <th scope="row"><button type="button" onClick={(event) => { event.stopPropagation(); onSelect(house.id) }}>{house.name}</button></th>
                  <td>{house.parcel}</td><td><StatusBadge status={house.status} /></td><td>{house.area} m²</td><td>{house.plot} m²</td><td>{house.rooms}</td><td>{formatPrice(house.price)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="homes-mobile-list" aria-label="Lista domów">
          {houses.map((house) => (
            <button key={house.id} className={activeId === house.id ? 'is-active' : ''} type="button" onClick={() => onSelect(house.id)}>

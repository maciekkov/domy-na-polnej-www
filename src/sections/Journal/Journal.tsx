import { ArrowRight, Bell, Images, Instagram, Mail } from 'lucide-react'
...
export function Journal({ entries }: { entries: JournalEntry[] }) {
  ...
  const submitNewsletter = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!email) return
    localStorage.setItem('dnp-newsletter-email', email)
      ...
      <div className="shell">
        ...
        <div className="journal-placeholder">
          <figure className="journal-placeholder__visual">
            <img src={`${import.meta.env.BASE_URL}assets/images/neighborhood/plots-aerial.webp`} alt="Teren inwestycji Domy na Polnej z lotu ptaka" width="1600" height="960" loading="lazy" />
            <figcaption><Bell aria-hidden="true" /><span><strong>Pierwsze aktualności już po rozpoczęciu budowy</strong><small>Tu pokażemy prawdziwy postęp prac — bez zdjęć zastępczych.</small></span></figcaption>
          </figure>
          <div className="journal-placeholder__content">
            ...
            <p>Gdy ruszą prace, pojawią się tu regularne zdjęcia i krótkie raporty z placu budowy. Bez fikcyjnych opinii i udawanych aktualności.</p>
            ...
            <form className="journal-newsletter" onSubmit={submitNewsletter}>
              <label htmlFor="journal-email"><Mail aria-hidden="true" /><span><strong>Powiadom mnie o starcie</strong><small>Jedna wiadomość, gdy pojawi się pierwszy wpis.</small></span></label>
              <div><input id="journal-email" type="email" value={newsletterEmail} onChange={(event) => { setNewsletterEmail(event.target.value); setNewsletterDone(false) }} placeholder="Twój adres e-mail" required /><button type="submit" aria-label="Zapisz się">Zapisz się <ArrowRight size={16} /></button></div>
              {newsletterDone && <p role="status">Dziękujemy — zapisaliśmy adres w wersji demonstracyjnej.</p>}
            </form>
          </div>
        </div>
      </div>

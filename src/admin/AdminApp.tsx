...
function Dashboard({ state, setPage }: { state: AdminState; setPage: (page: AdminPage) => void }) {
  ...
  return <>
    <div className="admin-metrics">
      ...
      <Metric icon={Gauge} label="Zmierzony ruch — 30 dni" value={`${new Set(events.map((item) => item.id.slice(0, 8))).size} sesji`} note="Wyłącznie po zgodzie analitycznej" />

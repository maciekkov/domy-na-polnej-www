...
import { publicSnapshot } from '../lib/sales.mjs'
...
export function AdminApp({ serverSession = false }: { serverSession?: boolean }) {
  const [authenticated, setAuthenticated] = useState(() => serverSession || sessionStorage.getItem(ADMIN_SESSION_KEY) === 'active')
  const [state, setState] = useState<AdminState>(() => readAdminState())
  const [page, setPage] = useState<AdminPage>(serverSession ? 'analytics' : 'dashboard')
  ...
  const publish = () => {
    ...
    try {
      ...
      const published = parseSiteData(publicSnapshot({ ...clone(state.draft), revision: revisionId, publishedAt: new Date().toISOString() }))
      localStorage.setItem(PUBLISHED_DATA_KEY, JSON.stringify(published))
      setState({
        ...state, draft: { ...state.draft, revision: published.revision }, published, dirty: false,
        revisions: [{ id: revisionId, createdAt: now(), summary: 'Zapis lokalnego szkicu', snapshot: clone(published), isCurrent: true }, ...state.revisions.map(item => ({ ...item, isCurrent: false }))],
      ...
      setNotice('Zapisano lokalny szkic. Wersja publiczna nie została zmieniona.')
    } catch (error) {
  ...
  const exportSiteData = () => {
    try {
      const data = parseSiteData(publicSnapshot({ ...state.draft, revision: Math.max(state.draft.revision, state.published.revision) + 1, publishedAt: new Date().toISOString() }))
  ...
  const logout = () => {
    if (serverSession) { window.location.assign('/administrator/?logout=1'); return }
    sessionStorage.removeItem(ADMIN_SESSION_KEY); setAuthenticated(false)
  }
  ...
  return (
    <div className="admin-app">
      <aside className={`admin-sidebar ${mobileMenu ? 'is-open' : ''}`}>
        ...
        <div className="admin-demo-badge"><ShieldCheck /> {serverSession ? 'Panel serwera' : 'Podgląd lokalny'}</div>
      ...
      <main className="admin-main">
        <header className="admin-topbar">
          ...
          {page !== 'analytics' && <div className="admin-topbar__actions">
            ...
            <button className="admin-button admin-button--primary" type="button" onClick={publish} disabled={!state.dirty}><Upload /> Zapisz szkic lokalnie</button>
          </div>}
        </header>
        {page !== 'analytics' && <div className="admin-safety-banner"><AlertTriangle /><div><strong>Lokalny szkic</strong><span>Zmiany w tej zakładce zapisują się tylko w tej przeglądarce. Eksport nie publikuje ich na serwerze.</span></div></div>}
        ...
        <div className="admin-content">
          ...
          {page === 'analytics' && <AnalyticsPanel serverSession={serverSession} />}
...
function HouseEditor({ house, history, onClose, onStatus, onSave, onPrice }: { house: House; history: AdminState['priceHistory']; onClose: () => void; onStatus: (status: HouseStatus) => void; onSave: (house: House) => void; onPrice: (price: number, reason: string) => void }) {
  ...
  const [newPrice, setNewPrice] = useState(house.price ?? 0)
  const [reason, setReason] = useState('')
  useEffect(() => { setForm(current => ({ ...current, price: house.price })); setNewPrice(house.price ?? 0) }, [house.price])

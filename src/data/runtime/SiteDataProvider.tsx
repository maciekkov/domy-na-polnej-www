...
function readPublished(): SiteData | null {
  ...
  try {
    const value = localStorage.getItem(PUBLISHED_DATA_KEY)
    return value ? JSON.parse(value) as SiteData : null
  } catch {
    return null
  }
}
...
export function SiteDataProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<SiteDataContextValue>(() => ({
    data: published ?? fallbackSiteData,
    source: published ? 'demo-published' : 'fallback',
  }), [published])

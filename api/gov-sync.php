...
$config=dnpConfig();
[$data]=dnpGuard('contact',8192,$config);dnpRequireAdmin($config);$action=(string)($data['action']??'status');
try{
  ...
  if($action==='enable'){
    ...
    dnpGovPayload($config); // Validate sales stage and complete public data before enabling.
